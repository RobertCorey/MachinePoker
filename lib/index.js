  ...
  MachinePoker = (function(superClass) {
    ...
    MachinePoker.prototype.addObserver = function(obs) {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        event = ref[i];
        if (obs[event]) {
          results.push(this.on(event, obs[event]));
        } else {
          results.push(void 0);
        }
      }
    ...
    MachinePoker.prototype.run = function() {
      ...
      game.once('complete', (function(_this) {
        return function(status) {
          ...
          _this.currentRound++;
          if (_this.currentRound % 10000 === 0) {
            console.log("\n\n\n\nround: " + _this.currentRound);
            console.log("====================================");
            _this.players.forEach(function(player) {
              return console.log(player.name + ": " + player.total);
            });
          }
          _this.players.forEach(function(player) {
            if (player.total) {
            ...
            return player.chips = 1000;
          });
          if (_this.currentRound > _this.maxRounds) {
